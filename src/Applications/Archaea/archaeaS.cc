...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  // Initiliaze potential body
	...
	LennardJonesFT ProteinPotential(0.0, LrestScaled); // No forces from protein potential during deflating

	// Then initialize potential body
  	double ScaledSearchR = PotentialSearchRF*AverageEdgeLength;
  	// PotentialBody ProteinBody(&MatSpring, defNodes, ScaledSearchR);
	PotentialBody ProteinBody(&ProteinPotential, defNodes, ScaledSearchR);
...
*/
  ...
  {
    x = defNodes[i]->point();
    if ( fabs(x[0]) > (1.0-tol) && fabs(x[1]) < tol && fabs(x[2]) < tol) {
      ...
      if (x[0] > 1.0-tol) {
	BoundType(idx[0]) = 3;
	UpperBound(idx[0]) = 1.0;
      }
      else {
	BoundType(idx[0]) = 1;
        LowerBound(idx[0]) = -1.0;
      }

    }
    else if ( fabs(x[0]) < tol && fabs(x[1]) > (1-tol) && fabs(x[2]) < tol) {
      idx = defNodes[i]->index();
      BoundType(idx[0]) = 2;
      ...
      if (x[1] > 1.0-tol) {
        BoundType(idx[1]) = 3;
        UpperBound(idx[1]) = 1.0;
      }
      else {
        BoundType(idx[1]) = 1;
        LowerBound(idx[1]) = -1.0;
      }
  ...
  if (!restart)
  {
    for(Vred = 0.99; Vred > 0.89; Vred -= 0.01)
    {
  ...
  // Activate protein potential ...
  ProteinPotential.setScaling(epsilon);
  // ... and equilibrate body before pulling
  solver.setBounds(BoundType, LowerBound, UpperBound);
  solver.solve(&model);
  ...
  // Reset PrintCount in case of restart
  PrintCount = 11;
