...
int main(int argc, char* argv[])
{
	...
	assert(fvkFile);
	std::vector<vector<double> > fvkVec;
	double currAm, currAp, currAn, currAc;
	while (fvkFile >> currAm >> currAp >> currAn >> currAc) {
		std::vector<double> currLine;
		currLine.push_back(currAm);
		currLine.push_back(currAp);
		currLine.push_back(currAn);
		currLine.push_back(currAc);
		fvkVec.push_back(currLine);
	}
	fvkFile.close();
	...
	//***************************  SOLUTION LOOP ***************************
	for(int z=0; z < fvkVec.size(); z++){
		currAm = fvkVec[z][0];
		bd->updateProperty(OPSBody::aM, currAm);
		currAp = fvkVec[z][1];
		bd->updateProperty(OPSBody::aP, currAp);
		currAn = fvkVec[z][2];
		bd->updateProperty(OPSBody::aN, currAn);
		currAc = fvkVec[z][3];
		bd->updateProperty(OPSBody::aC, currAc);
