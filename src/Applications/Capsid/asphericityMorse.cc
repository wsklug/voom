...
int main(int argc, char* argv[])
{
	...
	for (int q = 0; q < gammaVec.size(); q++) {
		...
		kdt->BuildLocatorFromPoints( lssPd );
		std::vector<Vector3D> oldValues;
		for(int pt=0; pt < defNodes.size(); pt++){
			double distVal;
			Vector3D oldVal(0.0), projVal(0.0);
			oldVal[0] = defNodes[pt]->getPoint(0);
			oldVal[1] = defNodes[pt]->getPoint(1);
			oldVal[2] = defNodes[pt]->getPoint(2);
			oldValues.push_back( oldVal );
			vtkIdType currId = kdt->FindClosestPoint( &(oldVal[0]),distVal);
			lssPd->GetPoint(currId, &(projVal[0]));
		...
		for(int pt=0; pt < oldValues.size(); pt++){
			defNodes[pt]->setPoint( oldValues[pt] );
		}
