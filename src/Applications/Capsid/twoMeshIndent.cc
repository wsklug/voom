...
int main(int argc, char* argv[]) {
	...
	assert(miscInpFile);
	miscInpFile >> temp >> epsilon >> temp >> percentStrain >> temp >> gamma_inp
	>> temp >> indent_inp >> temp >> step_inp >> temp >> friction_inp
	>> temp >> unload >> temp >> ARtol >> temp >> numSubDiv >> temp
	>> loopSurfSubDiv >> temp >> searchRadFactor >> temp
	>> capsoSearchRadFactor >> temp >> cleanTol;
	...
	vtkSmartPointer<vtkLinearSubdivisionFilter> lsdf = vtkSmartPointer<
	...
	std::vector<DeformationNode<3>*> bendingDefNodes;
	...
	if (verbose)
		cout << "Number of triangles in fine mesh: " << ntri << endl;
	for (int i = 0; i < ntri; i++) {
		assert(fineMesh->GetCell(i)->GetNumberOfPoints() == 3);
		for (int a = 0; a < 3; a++)
			c[a] = fineMesh->GetCell(i)->GetPointId(a);
		connectivities.push_back(c);
	}

	// read in triangle connectivities for coarseMesh
	vector<tvmet::Vector<int, 3> > coarseMeshConn;
	int coarseMeshTri = coarseMesh->GetNumberOfCells();
	coarseMeshConn.reserve(coarseMeshTri);
	if (verbose)
		cout << "Number of triangles in coarse mesh: " << coarseMeshTri << endl;
	for (int i = 0; i < coarseMeshTri; i++) {
		assert(coarseMesh->GetCell(i)->GetNumberOfPoints() == 3);
		for (int a = 0; a < 3; a++) {
			c[a] = coarseMeshPointIDs[coarseMesh->GetCell(i)->GetPointId(a)];
		}
	...
	lengthStat = calcEdgeLenAndStdDev(bendingDefNodes, coarseMeshConn);
	EdgeLength = lengthStat[0];
	stdDevEdgeLen = lengthStat[1];
	std::cout << "Before any relaxation :" << endl
			<< "   Average triangle edge length of coarse mesh = " << EdgeLength
			<< endl << "   Standard deviation = " << stdDevEdgeLen << endl;

	//Rescale the capsid such that coarse mesh triangle edge-lengths are
	//unity
	for (int i = 0; i < bendingNodes.size(); i++) {
		DeformationNode<3>::Point x;
		x = bendingDefNodes[i]->point();
		x *= 1.0 / EdgeLength;
		bendingDefNodes[i]->setPoint(x);
		bendingDefNodes[i]->setPosition(x);
	}
	...
	std::cout << "Morse  potential parameters:" << endl << "\tsigma = " << sigma
			<< endl << "\tPotentialSearchRF = " << PotentialSearchRF << endl
			<< "\tepsilon = " << epsilon << endl << "\tIs Remesh On? " << remesh
			<< endl << "\tARtol = " << ARtol << endl;

	// Protein body implemented using Morse potential body
	Morse Mat(epsilon, sigma, Rshift);

	// Then initialize potential body
	PotentialBody * PrBody = new PotentialBody(&Mat, morseNodes,
			PotentialSearchRF);
	...
	ViscosityBody * vb = new ViscosityBody(bendingDefNodes, connectivities,
			1e-2 * springConstant);
	...
	Model model(bdc, bendingNodes);

	Lbfgsb solver(model.dof(), m, factr, pgtol, iprint, maxIter);
	std::cout << "Relaxing shape for gamma = " << gamma << std::endl;

	// relax initial shape;
	int viscRegIterCount = 0;
	while (vb->energy() > 1e-4 * (bd->energy() + PrBody->energy())
			&& viscRegIterCount < viscRegIterMax) {
		solver.solve(&model);
		viscRegIterCount++;
	}

	std::cout << "Shape relaxed." << std::endl << "Energy = "
			<< solver.function() << std::endl;

	//Calculate centre of sphere as average of position vectors of all nodes.
	tvmet::Vector<double, 3> Xavg(0.0);
	for (int i = 0; i < bendingNodes.size(); i++) {
		Xavg += bendingDefNodes[i]->point();
	}
	...
	std::cout << "Effective 2D Young's modulus = " << Y << endl
			<< "FVK number = " << gamma << endl << "Asphericity = "
			<< asphericity << endl;
	...
	std::cout << "Zmax = " << Zmax << std::endl << "Zmin = " << Zmin
			<< std::endl;

	xc = 0.0, 0.0, Zmax + afmR;

	std::cout << "AFM Indenter radius =" << afmR << std::endl
			<< "AFM Indenter center = (" << xc[0] << "," << xc[1] << ","
			<< xc[2] << ")" << std::endl;
	...
	double k_AL = 1.0e2;
	RigidHemisphereAL * afm = new RigidHemisphereAL(bendingDefNodes, k_AL, afmR,
			xc, friction);
	afm->updateContact();
	...
	RigidPlateAL* glass = new RigidPlateAL(bendingDefNodes, k_AL, Z_glass, up,
			friction);
	bd->pushBack(glass);
	...
	FvsZ << "#Step\tIndentation\tGlass_Fz\tAFM_Fz"
			...
			<< "\tViscRegEnergy\tSolverFunction" << std::endl;
	...
	for (double Z = Zbegin;; Z += dZ, step++) {
		...
		if (verbose) {
			...
			std::cout << "       top active  = " << afm->active() << std::endl
					...
					<< "  top penetration  = " << afm->penetration()
					<< std::endl << "bottom penetration = "
					<< glass->penetration() << std::endl;
		}
		double height = Zmax - Z;

		if (verbose) {
			std::cout << "Contact converged." << std::endl << std::endl
					<< "        height = " << height << std::endl
					<< "   indentation = " << originalHeight - height
					<< std::endl << std::endl;
		}
		...
		if (remesh) {
			...
			if (elementsChanged > 0) {
				std::cout
				<< "Number of elements that changed after remeshing = "
		...
		FvsZ << step << "\t" << (originalHeight - height) / Ravg << "\t"
				...
				<< solver.function() / (Ravg * std::sqrt(Y * KC)) << std::endl;
		...
		vtkSmartPointer<vtkPolyData> morseParticles = vtkSmartPointer<
				vtkPolyData>::New();
		vtkSmartPointer<vtkPoints> morsePoints =
		...
		sstm << modelName << "-LoopShellSurf-" << step << ".vtk";
		rName = sstm.str();
	...
	std::cout << "Total execution time: " << diff / CLOCKS_PER_SEC << " seconds"
			<< std::endl;
	return 0;
