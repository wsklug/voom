...
int main(int argc, char* argv[])
{
  ...
  // Analysis Type
  int AnType = 0;        // 0 -> Shield analyses, fixed background, 
                         // 1 -> Shield analyses, random background, 
                         // 2 -> Shield analyses, zero background, 
                         // 3 -> Pairwise interaction with zero energy background, 
                         // 4 ->  Energy analyses in different randomly generated cases
  int NumEnergy = 1;     // number of energies to be computed for different three fold sites configurations - only works with AnType = 4;
  int ChangeConf    = 0;    // 0 -> One hexamer at the time is change in the background, 1 -> All hexamers are change at the same tiem
  int ShieldingConf = 0;    // 0 -> Random configuration changed from 002, 1-> Random configuration changed from 112.
  ...
  cout << " modelName      : " << modelName      << endl
       ...
       << " ChangeConf     : " << ChangeConf     << endl
       << " ShieldingConf  : " << ShieldingConf  << endl;
  ...
  PrintTvirus.setPrintingConfig(true);
   ...
   if (AnType == 0)  // Frustration case - Fixed Background (minimum energy case)
   {
    
     // Explore 81 cases for one selected triplet
       for (i = 0; i < TestShift.size(); i++) {
	 VAngleShift[NearNeighList[0]] = TestShift[i][0]*M_PI/3.0; 
	 VAngleShift[NearNeighList[1]] = TestShift[i][1]*M_PI/3.0;
	 VAngleShift[NearNeighList[2]] = TestShift[i][2]*M_PI/3.0;
	 VAngleShift[NearNeighList[3]] = TestShift[i][3]*M_PI/3.0; 
     ...
     // Testing far away shielding in 20 cases
       // Setting neighbor three fold sites in the 002 or 112 configuration
       if (ShieldingConf == 0)
       {
	 VAngleShift[NearNeighList[1]] = 0.0;
	 VAngleShift[NearNeighList[2]] = 0.0;
	 VAngleShift[NearNeighList[3]] = 2.0* M_PI/3.0;
         for (j = 0; j < stretch_angle.size(); j++) {
       ...
       else if (ShieldingConf == 1)
       {
	 VAngleShift[NearNeighList[1]] = M_PI/3.0;
	 VAngleShift[NearNeighList[2]] = M_PI/3.0;
	 VAngleShift[NearNeighList[3]] = 2.0* M_PI/3.0;
         for (j = 0; j < stretch_angle.size(); j++) {
	   if (CapsomersNum[j] == NearNeighList[1]) 
	   {
	     stretch_angle[j]             = M_PI/3.0;
	   }
	   else if (CapsomersNum[j] == NearNeighList[2]) 
	   {
	     stretch_angle[j]             = M_PI/3.0;
       ...
       // Loop through 20 randomly generated cases 
       srand(time(NULL));
       for (i=0; i<20; i++)
       {
	 // Change one Three fold site in the background at the time or all together
	 if (ChangeConf == 0)
	 {
	   ...
	   VAngleShift[*OtherTFSit] += randNumber*M_PI/3.0; 
	   for (j = 0; j < stretch_angle.size(); j++) {
	 ...
	 else if (ChangeConf == 1) // Change All three fold sites in the background at once
	 {
	 ...
	 // For each of the randomly generated configuration, loop over the 3 possible flips
	 for (k = 0; k < 3; k++) {
	   VAngleShift[NearNeighList[0]] = k*M_PI/3.0; 
	   for (j = 0; j < stretch_angle.size(); j++) {
	     if (CapsomersNum[j] ==  NearNeighList[0]) 
	     {
   ...
   else if (AnType == 1)  // Randomly generated background
   {
   ...
   else if (AnType == 2)  // Zero background case
   {
     // Set all other 3 fold site to have zero stretch
     ...
     // Testing far away shielding in 20 cases - Not meaningful for zero background case
        
   }
   else if (AnType == 3)  // Zero background case - Pairwise interactions
   {
     // Set all *20* 3 fold site to have zero stretch
     ...
     // Compute energy with no prestretch at all.
     // Solve 
     solver.solve(&model);
     double Wtot = bending_body->totalStrainEnergy() +  pentamer_body->totalStrainEnergy() + hex_body->totalStrainEnergy();
     cout << "State = " << 0 << " " << Wtot << endl; 
     cout << "Wconf = " <<  hex_body->totalConformationalEnergy() << endl;
     ...
     // Compute energy of state with one prestrech only (12 cases)
     for (i = 0; i < 4; i++) {
       stretchNodes[NearNeighList[i]]->setPoint(in_eta);
       for (k = 0; k < 3; k++) {
	 VAngleShift[NearNeighList[i]] = k*M_PI/3.0; 
	 for (j = 0; j < stretch_angle.size(); j++) {
	   if (CapsomersNum[j] ==  NearNeighList[i]) 
	   {
	     stretch_angle[j]             = k*M_PI/3.0;
	   }
	 }
	 ...
	 // Solve 
	 solver.solve(&model);
	 Wtot = bending_body->totalStrainEnergy() +  pentamer_body->totalStrainEnergy() + hex_body->totalStrainEnergy();
	 cout << "State = " << i*3 + k +1 << " " << Wtot << endl; 
	 cout << "Wconf = " << hex_body->totalConformationalEnergy() << endl;
	 
	 SymmIndex = PrintTvirus.printMaster(i*3 + k +1);
	 cout << "Symmetry Index " << endl;
	 cout << "Nodes index (hex) = " << std::setprecision(12) << SymmIndex[0] << endl;
	 cout << "Eta index   (hex) = " << std::setprecision(12) << SymmIndex[1] << endl;
	 cout << "Theta index (hex) = " << std::setprecision(12) << SymmIndex[2] << endl;
       ...
       stretchNodes[NearNeighList[i]]->setPoint(1.0); // Reset to zero prestretch AFTER solving
     }

     // Compute energy with pairwise interactions
     stretchNodes[NearNeighList[0]]->setPoint(in_eta);
     for (i = 1; i < 4; i++) {
       stretchNodes[NearNeighList[i]]->setPoint(in_eta);
       for (k = 0; k < 3; k++) {
	 // Set central hexamer
	 ...
	 for (uint m = 0; m < 3; m++) {
	   // Switch peripherical hexamers
	   VAngleShift[NearNeighList[i]] = m*M_PI/3.0; 
	   for (j = 0; j < stretch_angle.size(); j++) {
	     if (CapsomersNum[j] ==  NearNeighList[i]) 
	       {
		 stretch_angle[j]             = m*M_PI/3.0;
	       }
	   }
	   ...
	   solver.solve(&model);
	   Wtot = bending_body->totalStrainEnergy() +  pentamer_body->totalStrainEnergy() + hex_body->totalStrainEnergy();
	   cout << "STATE = " << (i-1)*9 + k*3 + m + 13 << " " << Wtot << endl; 
	   cout << "Wconf = " <<  hex_body->totalConformationalEnergy() << endl;
	 
	   SymmIndex = PrintTvirus.printMaster((i-1)*9 + k*3 + m + 13);
	   cout << "Symmetry Index " << endl;
	   cout << "Nodes index (hex) = " << std::setprecision(12) << SymmIndex[0] << endl;
	   cout << "Eta index   (hex) = " << std::setprecision(12) << SymmIndex[1] << endl;
	   cout << "Theta index (hex) = " << std::setprecision(12) << SymmIndex[2] << endl;
     ...
     // Computer energy with all 3 fold sites with prestretch = eta
     for (j = 0; j < stretchNodes.size(); j++) {
       if (OtherTFS.find(j) != OtherTFS.end()) 
       {
     ...
     // Compute energy with prestretch equal to eta
     // Solve 
     solver.solve(&model);
     Wtot = bending_body->totalStrainEnergy() +  pentamer_body->totalStrainEnergy() + hex_body->totalStrainEnergy();
     cout << "State = " << 40 << " " << Wtot << endl; 
     cout << "Wconf = " <<  hex_body->totalConformationalEnergy() << endl;
     
     SymmIndex = PrintTvirus.printMaster(40);
     cout << "Symmetry Index " << endl;
     cout << "Nodes index (hex) = " << std::setprecision(12) << SymmIndex[0] << endl;
     cout << "Eta index   (hex) = " << std::setprecision(12) << SymmIndex[1] << endl;
     cout << "Theta index (hex) = " << std::setprecision(12) << SymmIndex[2] << endl;
   ...
   {
     // Look at energy in NumEnergy randomly generated cases 
     srand(time(NULL));
     for (i=0; i<NumEnergy; i++)
     {
       cout << "State " << i << " ";
       // Select *20* random numbers (equivalent to the number of ALL 3 fold sites)
       OtherTFS.insert(3);   OtherTFS.insert(8);   OtherTFS.insert(24);   OtherTFS.insert(75);
