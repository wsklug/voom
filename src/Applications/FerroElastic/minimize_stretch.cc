...
int main(int argc, char* argv[])
{
  ...
  printAllParaview("Initial.vtk",  OutlineName, NPout, bdc, stretchNodes, directionNodes, CapsomersNum);
  printStretchDirectioParaview("StretchInitialDirections.vtk", meshFileName, bdc, AngleShift, HexonShift, stretchNodes, directionNodes, CapsomersNum, false);
  solver.solve(&model);
  printAllParaview("Minimized.vtk", OutlineName, NPout, bdc, stretchNodes, directionNodes, CapsomersNum);
  printStretchDirectioParaview("StretchFinalDirections.vtk", meshFileName, bdc, AngleShift, HexonShift, stretchNodes, directionNodes, CapsomersNum, true);
				...
				bool final)
{
  ...
  for(i = 0; i < NumHexon; i++) {
    ...
    // StretchLoc *= pow(eta,3);
    // StretchLocP /= pow(eta,3);

    
    // To not print stretches ~= 1.0
    if ( abs(eta-1.0) < 1.0e-4 ) {
      StretchLoc *= 0.0;
      StretchLocP *= 0.0;
    }
