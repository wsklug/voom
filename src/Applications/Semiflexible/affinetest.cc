...
typedef std::vector< AltNodePair > AltNodePairList;
typedef std::vector< AltNodePairList> AltNodePairData;
...
//   }
  ...
  if(argc == 3) {
    ...
    double L;
    std::cout << "Enter filament length: ";
    std::cin >> L;
    double affshear;
    std::cout << "Enter shear value: ";
    cin >> affshear;
    assert(affshear>0.0);
    double affmin;
    std::cout << "Enter minimum distance to look at: ";
    std::cin >> affmin;
    assert(affmin>0.0);
    double affmax;
    std::cout << "Enter maximum distance to look at: ";
    std::cin >> affmax;
    double affbinspace;
...
// 	pNameEnd = curString.find_first_of('\t',pNameBegin);
// 	parValStr.assign(curString,pNameBegin,pNameEnd-pNameBegin);
// 	if(parName.find("Affine_method")!=string::npos) affmethod.assign(parValStr);
// 	else if(parName.find("Affine_shear")!=string::npos) affshear = atof(parValStr.data());
// 	else if(parName.find("Affine_space")!=string::npos) affbinspace = atof(parValStr.data());
// 	else if(parName.find("Affine_tol")!=string::npos) affbintol = atof(parValStr.data());
// 	else if(parName.find("Affine_min")!=string::npos) affmin = atof(parValStr.data());
...
//     inFile.close();
    ...
    std::cout << "Confirming parameters: Lavg = " << L << ", shear = " << affshear << std::endl;

    char* unshearedFile = argv[1];
    std::string unshearedFileName(unshearedFile);
    char* shearedFile = argv[2];
    std::string shearedFileName(shearedFile);

    // read in data from unsheared file, looking for pairs of nodes with the right separation //
    int intSize = sizeof(int);
    int doubleSize = sizeof(double);

    std::ifstream inFileNoShear(unshearedFileName.data(), ios::in | ios::binary | ios::ate);
    std::cout << "Read in unsheared gel file." << std::endl;
    ifstream::pos_type fsize = inFileNoShear.tellg();
    std::cout << "File size = " << fsize << std::endl;
    
    // reset file to beginning //
    inFileNoShear.seekg(0,ios::beg);
    
    // read in system size and set up PeriodicBox //
    Vector2D sysSize;
    for(int i=0; i<2; i++) {
      double curSS;
      inFileNoShear.read((char *)(&curSS),doubleSize);
      sysSize[i] = curSS;
    }
    ...
    int nFils;
    inFileNoShear.read((char *)(&nFils),intSize);

    AltFilament fils[nFils];
    
    for(int j=0; j<nFils; j++) {
      int nNodesHere;
      inFileNoShear.read((char *)(&nNodesHere),intSize);
      fils[j].fnodes.reserve(nNodesHere);
      for(int k=0; k<nNodesHere; k++) {
	Vector2D curPos;
	for(int kdof=0; kdof<2; kdof++) {
	  double cpdof;
	  inFileNoShear.read((char *)(&cpdof),doubleSize);
	  curPos[kdof] = cpdof;
	}
	for(int kdof=0; kdof<2; kdof++) {
	  double rpdof;
	  inFileNoShear.read((char *)(&rpdof),doubleSize);
	}
	AltNode * newNode = new AltNode;
	newNode->pos = curPos;
	newNode->indx.first = j;
	newNode->indx.second = k;
	fils[j].fnodes.push_back(newNode);
      }
    }
    
    std::map< std::pair<int,int>, std::pair<int,int> > CLinkMap;
    // read in data on crosslinks //
    int nCL;
    inFileNoShear.read((char *)(&nCL),intSize);
    for(int cln=0; cln<nCL; cln++) {
      int slaveFil;
      int slaveNode;
      int mastFil;
      int mastNode;
      inFileNoShear.read((char *)(&slaveFil),intSize);
      inFileNoShear.read((char *)(&slaveNode),intSize);
      inFileNoShear.read((char *)(&mastFil),intSize);
      inFileNoShear.read((char *)(&mastNode),intSize);
      std::pair<int, int > slavePair;
      std::pair<int, int > mastPair;
      slavePair.first = slaveFil;
      slavePair.second = slaveNode;
      mastPair.first = mastFil;
      mastPair.second = mastNode;
      CLinkMap.insert(std::pair<std::pair<int, int>, std::pair<int ,int> >(slavePair,mastPair));
      CLinkMap.insert(std::pair<std::pair<int, int>, std::pair<int, int> >(mastPair,mastPair));
    }
    inFileNoShear.close();
    ...
    // now we must iterate through all node pairs and find pairs that are the right distance apart //
    AltNodePairList ndpairs[nPts];
    for(int f=0; f<nFils; f++) {
      if(f%100==0 && f!=0) std::cout << "Checked " << f << " filaments for pairs." << std::endl;
      for(int f2=f; f2<nFils; f2++) {
	bool clinked = false;
	int nd1 = 0;
	while(!clinked && nd1<fils[f].fnodes.size()) {
	  AltNode* n1 = fils[f].fnodes[nd1];
	  //if(box->inside(n1->pos)) {
	  if(1==1) {  
	    int nd2;
	    if(f2!=f) nd2 = 0;
	    else nd2 = nd1+1;
	    while(!clinked && nd2<fils[f2].fnodes.size()) {
	      AltNode* n2 = fils[f2].fnodes[nd2];
	      if(1==1) {
		//if(box->inside(n2->pos)) {
		// check to make sure nodes aren't linked //
		bool linked = false;
		if(CLinkMap.find(n1->indx) != CLinkMap.end() && CLinkMap.find(n2->indx) != CLinkMap.end()) {
		  if(CLinkMap[n1->indx] == CLinkMap[n2->indx]) {
		    linked = true;
		  }
		}
		if(!linked) {
		  Vector2D initDiff;
		  initDiff = n1->pos - n2->pos;
		  //box->mapDistance(initDiff);
		  double r = norm2(initDiff)/L;
		  if(r<rMax && r>=affmin-affbintol && fmod(r-affmin+affbintol,affbinspace) <= 2.0*affbintol) {
		    // determine proper bin for data point //
		    double tmpInd = (r-affmin+1.5*affbintol)/affbinspace;
    ...
    // now read in the sheared file and see where points are //
    std::ifstream inFileShear(shearedFileName.data(), ios::in | ios::binary | ios::ate);
    std::cout << "Read in sheared gel file." << std::endl;
    fsize = inFileShear.tellg();
    
    // reset file to beginning //
    inFileShear.seekg(0,ios::beg);
    
    // read in system size and set up PeriodicBox //
  
    for(int i=0; i<2; i++) {
      double curSS;
      inFileShear.read((char *)(&curSS),doubleSize);
    }
    
    // read in # of filaments
    int dummynFils;
    inFileShear.read((char *)(&dummynFils),intSize);
    
    for(int j=0; j<nFils; j++) {
      int nNodesHere;
      inFileShear.read((char *)(&nNodesHere),intSize);
      for(int k=0; k<nNodesHere; k++) {
	Vector2D ePos;
	for(int kdof=0; kdof<2; kdof++) {
	  double cpdof;
	  inFileShear.read((char *)(&cpdof),doubleSize);
	  ePos[kdof] = cpdof;
	}
	for(int kdof=0; kdof<2; kdof++) {
	  double rpdof;
	  inFileShear.read((char *)(&rpdof),doubleSize);
	}
	fils[j].fnodes[k]->endpos = ePos;
      }
    }
    inFileShear.close();
    
    for(int l=0; l<nPts; l++) {
      for(AltNodePairList::iterator li=ndpairs[l].begin(); li!=ndpairs[l].end(); li++) {
	AltNode* n1 = li->node1;
	AltNode* n2 = li->node2;
	if(1==1) {
	  //if(box->inside(n1->endpos) && box->inside(n2->endpos)) {
	  double thetaInit = li->initAngle;
	  Vector2D initDiff;
	  initDiff[0] = cos(thetaInit);
	  initDiff[1] = sin(thetaInit);
	  double thetaAff = atan2(initDiff[1],initDiff[0]+affshear*initDiff[1]);
	  Vector2D finalDiff;
	  finalDiff = n1->endpos - n2->endpos;
	  //box->mapDistance(finalDiff);
	  double thetaFinal = atan2(finalDiff[1],finalDiff[0]);
	  double dThetaDiff = (fabs((thetaFinal - thetaAff)) < 2.0*M_PI - fabs((thetaFinal - thetaAff))) ? thetaFinal - thetaAff : 2.0*M_PI - fabs(thetaFinal - thetaAff); 
	  double newDatPt = sqr(dThetaDiff);

	  data[l].mean += newDatPt; // update running total //
	  data[l].stddev += sqr(newDatPt); // update running total of squared value //
	  data[l].nDatPts++; // update total number of data points for this distance //
	}
      }
      ndpairs[l].clear();
    }
    ...
    std::cout << "#r/L\t<Dth^2>\tstd. dev.\tN_pts" << std::endl;

    for(DataSet::iterator di=data.begin(); di!=data.end(); di++) {
      std::cout << di->rL << "\t" << di->mean << "\t" << di->stddev << "\t" << di->nDatPts << std::endl;
    }
  }
}
