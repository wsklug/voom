...
int main(int argc, char* argv[]) {
  ...
  while(!inFile.eof()) {
	...
	pm.insert(pair<std::string,std::string>("maximum sep. for two body force calculation",parValStr));
      }
  ...
  Grid<Fil,2> * grid = new Grid<Fil,2>(box,gridSpaces);
...
//   }
  ...
  for(int i=0; i<nFils; i++) {
    ...
    bns.push_back(newNode);
    nodes.push_back(newNode);
    gel->addFilament(bns,kBond,kAngle,visc,kT,dt);
  }
...
//   }
  ...
  for(int ct=0; ct<maxIters/printStep; ct++) {
    std::cout << "Brownian Dynamics: " << ct*printStep << " time steps taken; energy = " << gel->energy() << std::endl;
    //bd->computeAndAssemble(false,false,true);
    char fname[256];
    sprintf(fname,"gelliq-%d",ct*printStep);
