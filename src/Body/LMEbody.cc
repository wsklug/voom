...
namespace voom
{
  ...
  LMEbody<Material_t,Shape_t>::LMEbody(Material_t material,
    ...
    _nodes(nodes), _beta(beta), _searchR(searchR), _tol(tol), _nItMax(nItMax) 
  {
    ...
    // initialize _nDOF
    _quadPoints.reserve(nodes.size());

    bool FindNeigh = true;
    int i = 0, j = 0, Nfailed = 0, n = 0, a = 0;
    for(ConstNodeIterator nIt = nodes.begin(); nIt != nodes.end(); nIt++)
    {
  ...
  template<class Material_t, class Shape_t >
  LMEbody<Material_t,Shape_t>::QuadPointStruct::
  ...
  //! Compute E0, E1, E2
  template<class Material_t, class Shape_t >
  void LMEbody<Material_t,Shape_t>::compute( bool f0, bool f1, bool f2 )
  {
...
#pragma omp parallel for			\
  ...
  //! Compute invariants
  template<class Material_t, class Shape_t >
  void LMEbody<Material_t, Shape_t>::cal_invariants( std::vector<double> & I1, std::vector<double> & I2, std::vector<double> & I3 )
  {
...
#pragma omp parallel for			\
  ...
  template<class Material_t, class Shape_t >
  void LMEbody<Material_t,Shape_t>::printParaview(const std::string name) const
  {
