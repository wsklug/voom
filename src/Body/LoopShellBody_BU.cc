...
//     ofs.open(fileName.c_str(), std::ios::out);
    ...
    uint LoopShellBody< Material_t >::Remesh(double ARtol, Material_t material, uint quadOrder)
  {
    ...
    std::vector<double > AspectRatios(_shells.size(), 0.0);
    for(uint i = 0; i < _shells.size(); i++)
    {
      ...
      AspectRatios[i] = (a*b*c)/(8.0*(s-a)*(s-b)*(s-c));
    }
    ...
    for (uint i = 0; i < _shells.size(); i++)
    {
      if (AspectRatios[i] > ARtol) // Min aspect ratio is 1.0 for equilateral triangles
      {
	...
	{
	  uint neighbor = Neighbors[i][j];
	  if ( (NewAspectRatios[j][0] < AspectRatios[i] && NewAspectRatios[j][1] < AspectRatios[i]) && // None of the new trianlges is worst than starting one
	       ...
	       ( (NewAspectRatios[j][0]*NewAspectRatios[j][1]) < (NewAspectRatios[indB[j]][0]*NewAspectRatios[indB[j]][1]) ) )
	  {
	    ...
	    AspectRatios[i] = NewAspectRatios[j][0];
	    AspectRatios[neighbor] = NewAspectRatios[j][1];
