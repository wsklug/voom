...
namespace voom
{
  ...
  void MFBody<Material_t,Shape_t>::initializeBody
  (Material_t material,
   ...
   const std::vector<double> & supp_size) 
  {
    ...
    std::cout << "MFBody::initializeBody(): dof = "<< _dof 
	      << std::endl;
    ...
    for(ConstMFNodeIterator n=_MFnodes.begin(); n!=_MFnodes.end(); n++) {
      ...
      std::vector<int> list;
      list=grid.GetList(p);
      shape.compute(p,_MFnodes,supp_size,list,radius);
      shape.checkPartitionOfUnity(p,_MFnodes);
      _quadPoints.push_back(QuadPointStruct(weight,material,shape));
      i++;
    }
  }
  ...
  template<class Material_t, class Shape_t >
  MFBody<Material_t,Shape_t>::QuadPointStruct::
  QuadPointStruct(double w, const Material_t & m, const Shape_t & s):weight(w),material(m)
  {
  ...
  //! compute function
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::compute( bool f0, bool f1, bool f2 )
  {
...
#pragma omp parallel for			\
  schedule(static) default(shared) 
	...
	for(int i=0; i<3; i++) {
      ...
      // compute energy
      if ( f0 )	_strainEnergy += material.energyDensity()*weight;
  ...
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::printParaview(const std::string name) const
  {
 ...
 //! create input file of output data used by Paraview, a 3D viewer
  template<class Material_t, class Shape_t>
  void MFBody<Material_t,Shape_t>::printParaviewPostProcess(const std::string name) const
  {
  ...
  //! set modulus
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::setModulus( std::vector<double> & C10, std::vector<double> & C01, std::vector<double> & D1 )
  {
    // sets the modulus for MooneyRivlin material (different at every node)
...
#pragma omp parallel for			\
  schedule(static) default(shared) 
	...
	p!=_quadPoints.end(); p++,i++){
      Material_t& material = p->material;
      material.setModulus(C10[i],C01[i],D1[i]);
    }
  ...
  //! compute invariants
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::cal_invariants( std::vector<double> & I1, std::vector<double> & I2, std::vector<double> & I3 )
  {
...
#pragma omp parallel for			\
  schedule(static) default(shared) 
	...
	F(2,0)*F(2,0) + F(2,1)*F(2,1) + F(2,2)*F(2,2);
      ...
      if(I3[inode]<0.) std::cout<<"The jacobian is negative at ."<<inode<<std::endl;
  ...
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::cal_stretch_ratios( std::vector<double> & lam1, std::vector<double> & lam2, std::vector<double> & lam3 )
  {
...
#pragma omp parallel for			\
  ...
  //! compute deformation gradient
  template<class Material_t, class Shape_t >
  void MFBody<Material_t,Shape_t>::cal_defGrad( std::vector<Tensor3D> & F)
  {
    // computes invariants of the deformation gradient at all the quadrature points (which for this class are assumed to be the same as nodes)
    if(_nodes.size()!=_quadPoints.size()){
      std::cout << "Number of nodes not equal to the number of quad points. Exiting"<<std::endl;
...
#pragma omp parallel for			\
  schedule(static) default(shared) 
	...
	const Vector3D & xa = _MFnodes[neighbours[a]]->point();
	for(int i=0; i<3; i++) {
	  Ftemp(i,0) += xa(i)*xDN[a];
	  Ftemp(i,1) += xa(i)*yDN[a];
	  Ftemp(i,2) += xa(i)*zDN[a];
	}
      }
      F[inode] = Ftemp;
      inode++;
