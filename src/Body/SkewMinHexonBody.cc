...
namespace voom
{
  SkewMinHexonBody::SkewMinHexonBody(const ConnectivityContainer & connectivities,
  ...
  MPI_Comm_rank( MPI_COMM_WORLD, &_processorRank );
    ...
    _active.reserve(Cs);
    if(connectivities.size() != shear_angle.size())
      cout << "SkewMinHexonBody::Error, number of elements and shear-angle are different." << endl;
	...
	nds.push_back(_membraneNodes[*n]);
      ...
      {
	EvansElastic_SkewedMin * Mat = new  EvansElastic_SkewedMin(mu, kS, ShearNodes[0]->point(), shear_angle[ElCount]+DirectionNodes[0]->point() );
	MemEl = new C0MembraneShear(nds, ShearNodes[0], DirectionNodes[0], shear_angle[ElCount], Quad, Mat, shape);
      }
      else
      {
	int cap = CapsomersNum[ElCount];
	EvansElastic_SkewedMin * Mat = new  EvansElastic_SkewedMin(mu, kS, ShearNodes[cap]->point(), shear_angle[ElCount]+DirectionNodes[cap]->point() );
	MemEl = new C0MembraneShear(nds, ShearNodes[cap], DirectionNodes[cap], shear_angle[ElCount], Quad, Mat, shape);
      }
    ...
    cout << "SkewMinHexonBody: initialized elements " << ElCount << endl;
  ...
  //! compute
  void SkewMinHexonBody::compute( bool f0, bool f1, bool f2 )
  {
    int i = 0;
  ...
  void SkewMinHexonBody::printParaview(const std::string name) const
  {
  ...
  void SkewMinHexonBody::printParaviewEigVec(const string name) const
  {
  ...
  //! create output file used by Paraview to display CELL vectors
  void SkewMinHexonBody::printParaview2(const string name) const
  {
    string fileName = name + "-eigvec.vtk";
    ofstream ofs(fileName.c_str());
    if (!ofs) {
