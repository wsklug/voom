...
{
	//! Constructor 1
	BrownianKickOPS::BrownianKickOPS(const NodeContainer &defNodes, double Cd,
    ...
    //Destructor
    BrownianKickOPS::~BrownianKickOPS(){}
    
    void BrownianKickOPS::updateSerialKick(){
        unsigned int randomNode = _dis->random();
        for(int i=0; i < _nodeCount; i++){
            if(i == randomNode){
                Vector3D xi(_rng.random(), _rng.random(),_rng.random());
	...
	std::vector<double> BrownianKickOPS::getKickStats()
	{
    ...
    void BrownianKickOPS::updateParallelKick(){
    ...
    /*
    void BrownianKickOPS::updateProjectedKick(){
        
        for(int i=0; i < _nodeCount; i++){
        	Vector6D xi(_rng.random(), _rng.random(),_rng.random(),
        	                	_rng.random(), _rng.random(),_rng.random());
        
            Vector6D currPoint = _nodes[i]->deformedDOFs();
            
            Vector6D tempVec( 0, 0, 0);
	...
	void BrownianKickOPS::truncatedProjectedKick() {
    ...
    void BrownianKickOPS::updateRotationKick(){
	...
	void BrownianKickOPS::update2DKick() {
		for (int i = 0; i < _nodeCount; i++) {
			Vector3D xi(_rng.random(),
				_rng.random(), 0.0);
			_delta_xB[i] = xi*sqrt(_D*_dt);
	...
	void BrownianKickOPS::update1DKick() {
		for (int i = 0; i < _nodeCount; i++) {
...
*/
    ...
    void BrownianKickOPS::compute(bool f0, bool f1, bool f2) {
