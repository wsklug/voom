...
#if !defined(__BrownianKickOPS_h__)
#define __BrownianKickOPS_h__
...
namespace voom
{
  ...
  //! BrownianKickOPS element provides a layer of random Brownian forces to a Body.
  /*! This class provides a simple way to implement Brownian
      dynamics. The idea is that we can implement viscous
      regularization and add a BrownianKickOPS element to a Body. The
      resultant energy resembles the energy of Brownian dynamics.
   */
 
  class BrownianKickOPS  : public Element
  {
    
  public:

    typedef OPSNode Node; // nickname for OPS nodes
    typedef std::vector< Node* > NodeContainer;
    typedef tvmet::Vector< double, 6 > Vector6D;

    //! Constructor
	BrownianKickOPS(const NodeContainer &nodes, double Cd, double D, double dt);
