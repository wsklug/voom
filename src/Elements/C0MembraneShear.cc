...
namespace voom
{
  C0MembraneShear::C0MembraneShear(NodeContainer & Nodes,
				   ...
				   bool InsertDirection)
    {
      ...
      if(InsertShear) {_baseNodes.push_back(ShearNode);};
      if(InsertDirection) {_baseNodes.push_back(DirectionNode);};
      _nodes = Nodes;
      
      _shearNode = ShearNode;
  ...
  void C0MembraneShear::compute(bool f0, bool f1, bool f2)
  {
	...
	const double shearForce = material->shearForce();
	...
	_shearNode->addForce(shearForce*weight);
	_directionNode->addForce(directionForce*weight);
        // cout << "sheraNode->force() = " << _shearNode->force() << endl;
  ...
  void C0MembraneShear::invariants(double& I1, double& J)
  {
  ...
  void C0MembraneShear::updateRefConfiguration() 
  {
    int b = 0;
    Vector3D zero(0);
  ...
  void C0MembraneShear::SetRefConfiguration(double edgelen) 
  {
    for(QuadPointIterator p = _quadPoints.begin(); p!=_quadPoints.end(); p++)
    {
      tvmet::Vector< Vector3D, 2 > a;
  ...
  Vector3D C0MembraneShear::computePosition(const double s1, const double s2)
  {
  ...
  const Tensor3D C0MembraneShear::cauchyStress()
  {
    Tensor3D sigma(0.0);
  ...
  const vector<double > C0MembraneShear::matInvariants()
  {
