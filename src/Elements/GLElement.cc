...
namespace voom
{
  ...
  void GLElement<Material_t>::compute(bool f0, bool f1, bool f2)
  {
    //
    // initialize things
    //
	...
	p!=_quadPoints.end(); p++){
      ...
      double eta  = 0.0;
      tvmet::Vector< double, 2 > dEta(0.0);
	...
	const double & etab = _nodes[b]->getPoint(3);
	...
	dEta(1) += DN(b,1) * etab; 
      ...
      material.setReactionCoordinate(eta, dEta); 
	...
	_inplaneEnergy += material.stretchingEnergy() * weight;       
      }
	...
	const tvmet::Vector< Vector3D, 3 >& glsr = material.GLStressResultants();
	const double&                      glr = material.GLResultants();
	const tvmet::Vector< double, 2 >& glgr = material.GLGradientResultants();
	...
	for (int a=0; a<_nodes.size(); a++) {
	  ...
	  for ( int alpha = 0; alpha < 2; alpha++){
	    ...
	    Vector3D ftmp;
	    ftmp = (sr(alpha) + glsr(alpha)) *  DN(a,alpha) * weight;
	  ...
	  _internalForce(a)(3) += glr * N(a)*weight;
	  for(int beta=0; beta<2; beta++){
	    _internalForce(a)(3) += glgr(beta)*DN(a,beta)*weight;
	  } 
...
// 	  _pressureForce(a) -= pressure * d * N(a) * weight;
	...
	double f_ia = _internalForce(a)(3); 
	(*na)->addForce( 3, f_ia );
  ...
  template<class Material_t>
  void GLElement<Material_t>::updateRefConfiguration() {
    for(QuadPointIterator p=_quadPoints.begin(); 
	p!=_quadPoints.end(); p++){
      //
      // compute Shell geometry
