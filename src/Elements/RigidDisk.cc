...
namespace voom
{
  ...
  RigidDisk::RigidDisk( const DefNodeContainer & nodes, 
					double k, double R, Vector2D xc, 
					double friction ) {
    _defNodes = nodes;
  ...
  void RigidDisk::compute( bool f0, bool f1, bool f2 ) {
	...
	if(f1) {
      ...
      const Vector2D pt = _frictionMultipliers(a);
	...
	if(f1) {
      ...
      if(f1) {
	const Vector2D & f = _forces(a); 
	_FZ += f(1);
	for(int i=0; i<2; i++) nd->addForce(i,f(i)); 
      }
