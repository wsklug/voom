...
namespace voom
{
	void ShellGeometry::_updateMetrics()
	{
		...
		_metricTensor(0,0) =  dot(_a(0),_a(0));
		_metricTensor(0,1) =  dot(_a(0),_a(1));
		_metricTensor(1,1) =  dot(_a(1),_a(1));
	...
	void ShellGeometry::_updateDualPartials()
	{
		...
		//
		_aDualPartials(0,0) = - (  _aDual(0) * dot( _aDual(0), _aPartials(0,0))
								   + _aDual(1) * dot( _aDual(0), _aPartials(1,0))
								   + _d        * dot( _aDual(0), _dPartials(0)) );
		//
		// alpha = 0, beta = 1, mu = 0, 1
		_aDualPartials(0,1) = - (  _aDual(0) * dot( _aDual(0), _aPartials(0,1))
								   + _aDual(1) * dot( _aDual(0), _aPartials(1,1)) 
								   + _d        * dot( _aDual(0), _dPartials(1)  ) );
		//
		// alpah = 1, beta = 0, mu = 0, 1
		_aDualPartials(1,0) = - (  _aDual(0) * dot( _aDual(1), _aPartials(0,0))
								   + _aDual(1) * dot( _aDual(1), _aPartials(1,0))  
								   + _d        * dot( _aDual(1), _dPartials(0)  ) );
		//
		// alpha = 0, beta = 1, mu = 0, 1
		_aDualPartials(1,1) = - (  _aDual(0) * dot( _aDual(1), _aPartials(0,1))
								   + _aDual(1) * dot( _aDual(1), _aPartials(1,1)) 
								   + _d        * dot( _aDual(1), _dPartials(1)  ) );
	...
	void ShellGeometry::_updateDirectorPartials()
	{
		...
		//
		_dPartials(0) = - (   _aDual(0) * dot( _d, _aPartials(0,0))
							  + _aDual(1) * dot( _d, _aPartials(1,0)) );
		_dPartials(1) = - (   _aDual(0) * dot( _d, _aPartials(0,1))
							  + _aDual(1) * dot( _d, _aPartials(1,1)) );
	}
