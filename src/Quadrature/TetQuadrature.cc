...
namespace voom {
  ...
  bool TetQuadrature::check(unsigned int d) const {
    ...
    // ** this was the triangle text: **
    // Integrate the polynomial exactly using the formula from Cook's text:
    //
    //   \int_A s_1^i s_2^j (1-s_1-s_2)^k dA = 2A\frac{i!j!m!}{(2+i+j+q)!}
    // 
    // or with k=0
    //
    ...
    // ** now for our tetrahedron: **
    // ************************************
    // ************************************
    // ************************************
    // HEY... OVER HERE!!!
    // LOOK!!!!
    // **** question: why is there no 2.0*a[k] above with the formula being 2A? 
    //      could this be anything to do with later the I_numerical being divided by 2? (I_numerical *= 0.5) ??
    // **** question: where exactly does the q++ for a[q] go in the I_exact loop below????? compare to a[k], k++ in above tri loop****
    //
    // Integrate the polynomial exactly using the formula from Cook's text:
    //
    //   \int_V s_1^i s_2^j s_3^k (1-s_1-s_2-s^3)^p dV = 6V\frac{i!j!k!q!}{(3+i+j+k+q)!}
    // 
    // or with q=0
    //
    //   \int_V s_1^i s_2^j s_3^k dV = 6V\frac{i!j!+k!}{(3+i+j+k)!}
    //
    // For the standard tet V=1.0/6.0
