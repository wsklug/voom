...
namespace voom
{
	...
	*/
	...
	public:
		...
		//! Default Constructor
		Lbfgsb(int n, int m = 5,
			double factr = 1.0e+1, double pgtol = 1.0e-5,
			int iprint = 0, int maxIterations = -1, bool debug = false)
			: _m(m), _factr(factr), _pgtol(pgtol),
			_iprint(iprint), _maxIterations(maxIterations), _debug(debug)
		{
			resize(n);
			_projg = 0.0;
		}
		...
		void setBounds(const IntArray & nbd,
