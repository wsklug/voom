  ...
  */
  ...
  public:
    ...
    MontecarloTwoStages(vector<vector<ScalarFieldNode<3>* > > & MontecarloDoF,
			...
			bool print = true): 
      _montecarloDoF(MontecarloDoF), _varType(VarType), _solver(GivenSolver), _nSteps(NSteps),  _print(print), _printingStretches(Printer)
    {
      assert(_montecarloDoF.size() == _varType.size());
      // Compute the number of dof to be changed in the Montecarlo solver
      _size = 0;
      for (unsigned int i = 0; i< _montecarloDoF.size(); i++)
      {
	if (VarType[i] >= 0)
	{
