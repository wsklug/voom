...
C
      INTEGER N,M,IPRINT(2),IFLAG,DIAGCO
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
      DOUBLE PRECISION GTOL,ONE,ZERO,GNORM,UBDDOT,STP1,FTOL,STPMIN,
...
C
         YS= UBDDOT(N,W(IYPT+NPT+1),1,W(ISPT+NPT+1),1)
      IF(DIAGCO.EQ.0) THEN
         YY= UBDDOT(N,W(IYPT+NPT+1),1,W(IYPT+NPT+1),1)
C PARALLELIZED
!$OMP PARALLEL DO
!$OMP& DEFAULT(SHARED) PRIVATE(I)
!$OMP& SCHEDULE(STATIC)
...
C PARALLELIZED
      ...
      DO 125 I= 1,BOUND
         CP=CP-1
         IF (CP.EQ. -1)CP=M-1
         SQ= UBDDOT(N,W(ISPT+CP*N+1),1,W,1)
         INMC=N+M+CP+1
         IYCN=IYPT+CP*N
         W(INMC)= W(N+CP+1)*SQ
         CALL UBDAXPY(N,-W(INMC),W(IYCN+1),1,W,1)
 125  CONTINUE
C
C PARALLELIZED
!$OMP PARALLEL DO
...
C
      DO 145 I=1,BOUND
         YR= UBDDOT(N,W(IYPT+CP*N+1),1,W,1)
         BETA= W(N+CP+1)*YR
         INMC=N+M+CP+1
         BETA= W(INMC)-BETA
         ISCN=ISPT+CP*N
         CALL UBDAXPY(N,BETA,W(ISCN+1),1,W,1)
         CP=CP+1
         IF (CP.EQ.M)CP=0
 145  CONTINUE
C
...
C   ----------------------------------------------------------
C
      subroutine ubdaxpy(n,da,dx,incx,dy,incy)
c
...
c
      ...
      integer i,incx,incy,ix,ix0,iy,iy0,m,mp1,n
c
      if(n.le.0)return
...
c
      ix0 = 1
      iy0 = 1
      if(incx.lt.0)ix0 = (-n+1)*incx + 1
      if(incy.lt.0)iy0 = (-n+1)*incy + 1
C PARALLELIZED
!$OMP PARALLEL DO
!$OMP& DEFAULT(SHARED) PRIVATE(i,ix,iy)
!$OMP& SCHEDULE(STATIC)
      do 10 i = 1,n
        ix = (i-1)*incx + ix0
        iy = (i-1)*incy + iy0
...
C   ----------------------------------------------------------
C
      double precision function ubddot(n,dx,incx,dy,incy)
c
...
c
      ...
      if(incx.lt.0)ix0 = (-n+1)*incx + 1
      if(incy.lt.0)iy0 = (-n+1)*incy + 1
C PARALLELIZED, EACH WITH OWN DTEMP, THEN COLLECT
!$OMP PARALLEL DO
!$OMP& DEFAULT(SHARED) PRIVATE(i,ix,iy)
!$OMP& SCHEDULE(STATIC)
!$OMP& REDUCTION(+:dtemp)
...
C
      DGINIT = ZERO
C PARALLELIZED, EACH WITH OWN DGINIT, THEN COLLECT
!$OMP PARALLEL DO
!$OMP& DEFAULT(SHARED) PRIVATE(J)
!$OMP& SCHEDULE(STATIC)
!$OMP& REDUCTION(+:DGINIT)
      DO 10 J = 1, N
