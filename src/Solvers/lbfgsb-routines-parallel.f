...
c     ************
      ...
      character*3      word
      integer          i,im,k,nintol,itfile,iback,nskip,
...
C         write(6,*) 'Checking for errors in input args...'
	 call errclb(n,m,factr,l,u,nbd,task,info,k)
         if (task(1:5) .eq. 'ERROR') then
C            write(6,*) '... found errors'
...
c PARALLELIZED
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(k,k1)
!$OMP DO SCHEDULE(STATIC)
!$OMP& REDUCTION(+:temp1)
	    do 15 k = pbegin, pend
	       k1 = ind(k)
...
c               write (itfile,3002)
c     +             iter,nfgv,nint,nact,word,iback,stp,xstep
               write (itfile,3002)
     +             iter,
     +             nfgv,
     +             nint,
...
c PARALLELIZED
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(i,iy,ix)
!$OMP DO SCHEDULE(STATIC)
      do 10 i = 1,n
        ix = ix0 + (i-1)*incx
        iy = iy0 + (i-1)*incy
...
c
      ...
      if(incy.lt.0)iy0 = (-n+1)*incy + 1
c PARALLELIZED
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(i,ix,iy)
!$OMP DO SCHEDULE(STATIC)
      do 10 i = 1,n
        ix = ix0 + (i-1)*incx
        iy = iy0 + (i-1)*incy
...
c
c         ldt       integer
cT                   ldt is the leading dimension of the array t.
c
c         n         integer
